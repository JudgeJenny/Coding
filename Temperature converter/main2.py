...
while keepgoing == True:
    ...
    while isnumber(direction) == False or float(direction) not in (1,2):
        direction = input("That's not a valid input. Please enter 1 or 2: ").strip()

    # Get user input on the temperature they would like to convert. This must be a valid number.
    temp = input("What temperature would you like to convert? ").strip()

    # Confirm that valid input was received. If not, prompt user to enter again.
    while isnumber(temp) == False:
        temp = input("That's not a valid temperature. Please enter a number: ").strip()
    ...
    # Check that valid input was received. If not, prompt user again.
    while again not in ("y", "n"):
        again = input("That's not a valid input. Please enter Y or N: ").strip().lower()
    
    # Turn off the converter if user does not want to convert again
    if again == "y":
        keepgoing = True
    elif again == "n":
        keepgoing = False
